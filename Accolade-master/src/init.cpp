...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Accolade.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Accoladed.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeAccoladeamount=<n> " + _("Keep N Accolade anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Accolade is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Accolade is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Accolade version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Accolade server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Accolade") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Accolade to complete") << "\n";
    ...
    nAnonymizeAccoladeAmount = GetArg("-anonymizeAccoladeamount", 0);
    if(nAnonymizeAccoladeAmount > 999999) nAnonymizeAccoladeAmount = 999999;
    if(nAnonymizeAccoladeAmount < 2) nAnonymizeAccoladeAmount = 2;
    ...
    LogPrintf("Anonymize Accolade Amount %d\n", nAnonymizeAccoladeAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1ACCO+1000 == (.1ACCO+100)*10
       10ACCO+10000 == (1ACCO+1000)*10
    */
