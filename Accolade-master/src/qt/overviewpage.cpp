...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAccoladeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAccoladeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAccoladeAmount = strAnonymizeAccoladeAmount.remove(strAnonymizeAccoladeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAccoladeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAccoladeAmount*COIN) nMaxToAnonymize = nAnonymizeAccoladeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAccoladeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAccoladeAmount));
        strAnonymizeAccoladeAmount = strAnonymizeAccoladeAmount.remove(strAnonymizeAccoladeAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAccoladeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
