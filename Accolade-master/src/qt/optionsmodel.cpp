...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAccoladeAmount"))
        settings.setValue("nAnonymizeAccoladeAmount", 1000);
    nAnonymizeAccoladeAmount = settings.value("nAnonymizeAccoladeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAccoladeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAccoladeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAccoladeAmount:
            return QVariant(nAnonymizeAccoladeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAccoladeAmount:
            nAnonymizeAccoladeAmount = value.toInt();
            settings.setValue("nAnonymizeAccoladeAmount", nAnonymizeAccoladeAmount);
            emit AnonymizeAccoladeAmountChanged(nAnonymizeAccoladeAmount);
            break;
        default:
            break;
        }
